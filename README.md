# İkbal Torun
**Software Engineering Student @ Karabük University**

---

### Hakkımda
Karabük Üniversitesi'nde Yazılım Mühendisliği 3. sınıf öğrencisiyim. Backend sistemler, web geliştirme ve oyun simülasyonları üzerine projeler geliştiriyorum. Temiz kod (clean code) prensiplerini ve modern DevOps süreçlerini öğrenme ve uygulama sürecindeyim.

### Teknik Yetkinlikler
* **Diller:** C#, Python, Java, C, SQL
* **Web:** HTML5, CSS3, JavaScript
* **Teknolojiler:** Unity 6, Docker, Jenkins, SonarQube

### Öne Çıkan Projeler
* **Dijital Davetiye:** Modern web teknolojileriyle geliştirilmiş, LCV entegreli etkinlik yönetim sistemi.
* **DevOps Pipeline:** Docker, Jenkins ve SonarQube entegrasyonu ile CI/CD süreç yönetimi.

---
**İletişim:** ikbaltorunn@email.com | [LinkedIn] https://www.linkedin.com/in/ikbaltorun/
